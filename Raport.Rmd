---
title: "International Hamster Race Federation"
author: "Krystian Andrzejewski, Kajetan Marszałek, Franciszek Zuter, Filip Pilarski"
date: "2026-01-26"
output: 
   html_document:
...
#wczytanie pakietu potrzebnego do połączenia
library(RMariaDB)

#ustanowienie połączenia z serwerem zajęciowym
...
kable(df1, col.names=c('Kategoria', 'Imię chomika', 'Właściciel', 'Liczba zwycięstw', 'Pozycja'))
...
kable(df2, col.names=c('Średnia długość szczytu kariery', 'Rekordowa długość szczytu kariery'), caption='Średnia i rekordowa długość szczytu kariery w dniach')


query3 <- "SELECT 
    -- W jakim wieku zazwyczaj zdobywa się złoto?
    ROUND(AVG(TIMESTAMPDIFF(MONTH, h.birth_date, c.date)), 1) AS Sredni_Wiek_Mistrza_Msc,
    
    -- Kiedy najwcześniej zaczynają wygrywać?
    MIN(TIMESTAMPDIFF(MONTH, h.birth_date, c.date)) AS Wiek_Wschodzacej_Gwiazdy_Msc,
    
    -- Kiedy najpóźniej wygrywają (koniec szczytu)?
    MAX(TIMESTAMPDIFF(MONTH, h.birth_date, c.date)) AS Wiek_Weterana_Msc
FROM participations p
JOIN competitions c ON p.competition_id = c.competition_id
JOIN hamsters h ON p.hamster_id = h.hamster_id
WHERE p.place = 1 AND p.disqualified = 0;"
...
kable(df3, col.names=c('Średni wiek mistrza', 'Wiek wschodzącej gwiazdy', 'Wiek weterana'), caption='Wiek chomików na poszczególnych etapach kariery w miesiącach')
```
# Pytanie 2
## Czy któraś kategoria daje przewagę bogatym właścicielom chomików, którzy mogą sobie pozwolić na lepszy sprzęt?
```{r pytanie2_kategorie}
...
SELECT
  ...
  wealth_group AS grupa_majatkowa,
  COUNT(*) AS liczba_wlascicieli,
  SUM(starts) AS liczba_startow,
  AVG(avg_place) AS srednie_miejsce,
  AVG(win_rate) AS odsetek_zwyciestw,
  AVG(podium_rate) AS odsetek_podium
FROM owner_category_grouped
WHERE avg_place IS NOT NULL
GROUP BY kategoria, grupa_majatkowa
ORDER BY srednie_miejsce,
  FIELD(grupa_majatkowa, 'biedni (1-3)', 'sredni (4-7)', 'bogaci (8-10)');
")
kable(df1, col.names=c('Kategoria', 'Grupa majątkowa', 'Liczba właścicieli', 'Liczba startów', 'Średnie miejsce', 'Odsetek zwycięstw', 'Odsetek podium'))


```
## Czy któraś dyscyplina daje przewagę bogatym właścicielom chomików, którzy mogą sobie pozwolić na lepszy sprzęt?
```{r pytanie2_dyscypliny}
...
SELECT
discipline_name AS dyscyplina,
category AS kategoria,
poor_owners AS liczba_biednych_wlascicieli,
rich_owners AS liczba_bogatych_wlascicieli,
avg_place_poor AS srednie_miejsce_biedni,
avg_place_rich AS srednie_miejsce_bogaci,
(avg_place_poor - avg_place_rich) AS przewaga_bogatych
FROM agg
WHERE poor_owners >= 2
AND rich_owners >= 2
ORDER BY przewaga_bogatych DESC
LIMIT 10;")
kable(df2, col.names=c('Dyscyplina', 'Kategoria', 'Liczba biednych właścicieli', 'Liczba bogatych właścicieli', 'Średnie miejsce biednych', 'Średnie miejsce bogatych', 'Przewaga bogatych'))
...
```{r pytanie4.1_miesiac}
df3 <- dbGetQuery(con, "
SELECT
MONTHNAME(h.birth_date) AS miesiac_urodzenia,
COUNT(*) liczba_startow,
AVG(CASE 
    WHEN p.disqualified = 0 
    AND p.place IS NOT NULL 
    THEN p.place 
    END) AS srednie_miejsce
FROM hamsters h
JOIN participations p ON p.hamster_id = h.hamster_id
GROUP BY miesiac_urodzenia
HAVING liczba_startow >= 10
ORDER BY srednie_miejsce ASC;")

# tłumaczenie nazw miesięcy dla lepszej przejrzystości 
miesiace_pl <- c(
  January   = "styczeń",
  February  = "luty",
  March     = "marzec",
...
df3$miesiac_urodzenia <- miesiace_pl[df3$miesiac_urodzenia]

kable(df3, col.names=c('Miesiąc urodzenia', 'Liczba startów', 'Średnie miejsce'))
```
## Czy chomiki z większej hodowli lepiej radzą sobie na zawodach?
```{r pytanie4.2_hodowle}
df4 <- dbGetQuery(con, "
WITH owner_stats AS (
  SELECT
    o.owner_id,
...
SELECT
  hamster_count AS liczba_chomikow_u_wlasciciela,
  owners AS liczba_wlascicieli,
  starts_total AS liczba_startow,
  mean_avg_place AS srednie_miejsce
FROM grouped
ORDER BY liczba_chomikow_u_wlasciciela;
")
kable(df4, col.names=c('Liczba chomików u właściciela', 'Liczba właścicieli', 'Liczba startów', 'Średnie miejsce'))
```
...
SELECT
  CONCAT(FLOOR(weight / 20) * 20, '-', FLOOR(weight / 20) * 20 + 19) AS przedzial_wagi,
  COUNT(*) AS liczba_startow,
  COUNT(DISTINCT hamster_id) AS liczba_chomikow,
  AVG(CASE WHEN disqualified = 0 AND place IS NOT NULL THEN place END) AS srednie_miejsce,
  AVG(CASE WHEN disqualified = 0 AND place = 1 THEN 1 ELSE 0 END) AS odsetek_zwyciestw,
  AVG(CASE WHEN disqualified = 0 AND place <= 3 THEN 1 ELSE 0 END) AS odsetek_podium
FROM perf
GROUP BY przedzial_wagi
HAVING liczba_startow >= 10
ORDER BY srednie_miejsce ASC;")

kable(df5, col.names=c('Przedział wagi', 'Liczba startów', 'Liczba chomików', 'Średnie miejsce', 'Odsetek zwycięstw', 'Odsetek podium'))
```
## Czy konkretne specjalności (specialty) dają przewagę na zawodach?
```{r pytanie4.4_specjalnosc}
df6 <- dbGetQuery(con, "
SELECT
  COALESCE(h.specialty, 'brak_specjalnosci') AS specjalnosc,
  COUNT(*) AS liczba_startow,
  COUNT(DISTINCT h.hamster_id) AS liczba_chomikow,
  AVG(CASE WHEN p.disqualified = 0 AND p.place IS NOT NULL THEN p.place END) AS srednie_miejsce,
  AVG(CASE WHEN p.disqualified = 0 AND p.place = 1 THEN 1 ELSE 0 END) AS odsetek_zwyciestw,
  AVG(CASE WHEN p.disqualified = 0 AND p.place <= 3 THEN 1 ELSE 0 END) AS odsetek_podium
FROM hamsters h
JOIN participations p ON p.hamster_id = h.hamster_id
...
kable(df6, col.names=c('Specjalność', 'Liczba startów', 'Liczba chomików', 'Średnie miejsce', 'Odsetek zwycięstw', 'Odsetek podium'))
